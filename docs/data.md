...
- **Pre-2023 release (2018–2022 + legacy):** columns are `fips_code`, `county_name`, `state`, `sum` (customers_out), `run_start_time`. **No `total_customers` column.**
- **2023+ release:** canonical schema `fips_code`, `county_name`, `state`, `customers_out`, `total_customers`, `run_start_time`.

`src/data/eagle_i.py` handles both schemas transparently. For pre-2023 records, the missing `total_customers` is imputed at panel-construction time from the per-FIPS median of years where it is present (effectively 2024). The customer count is constant within a county across all 15-min intervals in the 2024 raw data, so this is a defensible substitute. Without this imputation, `outage_fraction` and `outage_event_flag` would be `NaN` for 2018–2022 and the historical regression would collapse to a single year.
